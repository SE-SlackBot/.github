...
날씨·강의 시간표·Google Calendar를 지정 시각에 Slack으로 보내는 단일 워크스페이스 개인 봇. 시간표는 슬래시 커맨드로 관리하고 ICS로 구독한다. 사용자 안내는 [README](https://github.com/SE-SlackBot/main/blob/main/README.md), 운영은 [DEPLOY](reference/deploy.md)를 따른다.
...
- 명령어·별칭·인자 계약은 [commands.py](https://github.com/SE-SlackBot/main/blob/main/bot/commands.py)의 `*_COMMANDS`, `UPDATE_FIELD_ALIASES`, `DAY_ALIASES`, `_parse_*`가 원본이다. 숫자 `1` 접미 별칭은 중복 명령명 회피용이며 도움말에는 없다. 별칭 추가 시 Slack App에도 등록한다.
...
- DB 계약은 [courses.py](https://github.com/SE-SlackBot/main/blob/main/bot/courses.py)의 `init_db`, [config_store.py](https://github.com/SE-SlackBot/main/blob/main/bot/config_store.py)의 `CREATE_USER_CONFIG`, [database.py](https://github.com/SE-SlackBot/main/blob/main/bot/database.py)의 `CREATE_GOOGLE_TOKENS`가 원본이다. `courses` 소유자 기본값은 `default`, `user_config`·`google_tokens` 키는 `slack_user_id`다.
...
- Google OAuth 토큰 원문은 `DATABASE_URL`이 있으면 DB, 아니면 `GOOGLE_TOKEN_DIR` 파일에 저장한다. 이 선택은 import 시 고정되어 런타임 환경변수 변경은 반영하지 않는다. 로컬 인증은 [google_calendar.py](https://github.com/SE-SlackBot/main/blob/main/bot/google_calendar.py)의 `authorize_user`와 경로 상수를 따른다.
- 필수 환경변수 누락·`NOTIFY_TIME` 형식 오류는 `sys.exit(1)`, 그 외 기동 실패는 로그 후 진행한다. 필수값·기본값 원본은 [main.py](https://github.com/SE-SlackBot/main/blob/main/main.py), 설정 안내는 [환경 변수](reference/deploy.md#환경-변수)다. `courses`가 비면 부팅 시 `default` 샘플 6건을 재삽입하므로 빈 상태가 유지되지 않는다.
